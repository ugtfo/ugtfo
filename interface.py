...
def create_label(root, str, coordinates):
    label = Label(root, text=str, bg="light gray", width=25,
                  font="Verdana 12")
    label.place(x=coordinates[0], y=coordinates[1], anchor="center", width=400)
...
def selection(count, list_text, coordinates):
    ...
    for i in range(count):
        method = Radiobutton(text=list_text[i], variable=var,
                             value=i, bg="light gray", width=25, font="Verdana 12")
...
def create_list_box(root, coordinates):
    list_box = Listbox(root, width=48, height=17, bg="light gray")
