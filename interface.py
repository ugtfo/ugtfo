...
def selection(count, list_text, coordinates):
    ...
    for i in range(count):
        method = Radiobutton(text=list_text[i], variable=var,
                             value=i, bg="white", width=25, font="white")
...
def create_list_box(root, coordinates):
    list_box = Listbox(root, width=48, height=17, bg="white")
    list_box.insert(END, "(x y)")
    #list_box.place(x=coordinates[0], y=coordinates[1])
    list_box.place(x=coordinates[0] + 50, y=coordinates[1] - 70)
    scroll = Scrollbar(command=list_box.yview)
    #scroll.place(x=coordinates[0] + 490, y=coordinates[1], height=310)
    scroll.place(x=coordinates[0] + 490 + 50, y=coordinates[1] - 70, height=310)
    return list_box
